// Dictionary read — a 404 is the capability's declared not_found (a terminal
// failed state). Surfaces the first pronunciation clip as a portable artifact.
  ...
  if (mode === "simulated") {
    return {
      output: {
        ...
        entries: [
          { word: word || "serendipity", meanings: [{ partOfSpeech: "noun", definitions: [{ definition: "simulated definition" }] }] },
        ],
      },
      artifacts: [],
      evidence: [{ type: "simulated_execution", note: "canned entry, no upstream call.", observed_at: nowIso() }],
    };
  }
